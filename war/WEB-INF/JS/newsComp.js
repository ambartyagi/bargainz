...
casper.start("http://www.ndtv.com/", function() {
    nbLinks = this.evaluate(function() {
        return __utils__.findAll('ul > li').length;
    });
    ...
    var image;
    image = "ndtvshot" + "biggestImage" + ".png";
    images.push(image);
    this.echo("Processing image " + "biggestImage");
    this.captureSelector(image, '.bigst_img');
});
...
next = function() {
    var image;
    image = "ndtvshot" + currentLink + ".png";
    images.push(image);
    this.echo("Processing image " + currentLink);
    try{
       this.captureSelector(image, 'data-vr-contentbox');
    }catch(err){
       this.echo(err);
    }
